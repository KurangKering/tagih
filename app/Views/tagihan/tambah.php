...
<template  id="render-action-button-template">
	<div class="btn-group" style="white-space: nowrap">
		<button type="button" class="btn  btn-outline-info btn-edit" onclick="show_edit_mahasiswa_modal('place_here')"><i class="fas fa-plus"></i> Tambah</button>
	</div>
</template>
<?= $this->endSection() ?>
...
<script>
	...
	$(function() {
		...
		$(document).on('change', '.filters', function(e) {
			...
			.done(function(result) {
				...
				$("#mahasiswa_table").DataTable().rows.add(result);
				$("#mahasiswa_table").DataTable().draw();
		...
		$("#button_tambah_tagihan").click(function(e) {
			...
			if ($("#mahasiswa_table").DataTable().columns(0).data()[0].length > 0) {
	...
	function show_tambah_mahasiswa_modal() {
		...
		mahasiswaPeriodeIds = $("#mahasiswa_table").DataTable().columns(0).data()[0];
